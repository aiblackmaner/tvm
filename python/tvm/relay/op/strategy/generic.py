...
def conv3d_strategy(attrs, inputs, out_type, target):
    ...
    return strategy
# modify
@override_native_generic_func("conv3d_NCDHWc_strategy")
def conv3d_NCDHWc_strategy(attrs, inputs, out_type, target):
    """conv2d_NCHWc generic strategy"""
    logger.warning("conv3d_NCDHWc is not optimized for this platform.")
    strategy = _op.OpStrategy()
    strategy.add_implementation(
            wrap_compute_conv3d(topi.nn.conv3d_NCDHWc, True),
            wrap_topi_schedule(topi.generic.schedule_conv3d_NCDHWc),
            name="conv3d_NCDHWc.generic",
        )
    return strategy
# end
