...
def conv3d_strategy_cpu(attrs, inputs, out_type, target):
    ...
    # if layout == "NCDHW":
    #     strategy.add_implementation(
    #         wrap_compute_conv3d(topi.x86.conv3d_NCDHWc),
    #         wrap_topi_schedule(topi.x86.schedule_conv3d_NCDHWc),
    #         name="conv3d_NCDHWc.x86",
    #     )
    # end
    if layout == "NCDHW":
        strategy.add_implementation(
    ...
    return strategy
# modify
@conv3d_NCDHWc_strategy.register("cpu")
def conv3d_NCDHWc_strategy_cpu(attrs, inputs, out_type, target):
    """conv3d generic strategy"""
    strategy = _op.OpStrategy()
    layout = attrs.data_layout
    strategy.add_implementation(
        wrap_compute_conv3d(topi.x86.conv3d_NCDHWc, True),
        wrap_topi_schedule(topi.x86.schedule_conv3d_NCDHWc),
        name="conv3d_NCDHWc.x86",
    )
    return strategy
