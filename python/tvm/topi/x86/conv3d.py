...
from ..util import get_const_tuple, simplify, get_const_int, tag
from .util import get_fp32_len
from ..nn.conv3d import conv3d_infer_layout
...
#     # modify
#     packed_out = _conv3d_NCDHWc(cfg, data, kernel, strides, padding, dilation, layout, out_dtype)
...
def _pack_data(cfg, data, kernel):
    n, _, id, ih, iw = get_const_tuple(data.shape)
    oc, ic, kd, kh, kw = get_const_tuple(kernel.shape)
    ...
    if isinstance(n, tvm.tir.Any):
        n = tvm.te.size_var("n")
    if isinstance(id, tvm.tir.Any):
        id = tvm.te.size_var("id")
    ...
    if isinstance(ic, tvm.tir.Any):
        raise RuntimeError("Dynamic input channel is not supported for conv3d.")
    dshape = (n, ic_chunk, id, ih, ic_bn, iw)
    kshape = (oc_chunk, ic_chunk, kd, kh, kw, ic_bn, oc_bn)
    data_vec = te.compute(
        dshape, lambda n, C, d, h, c, w: data[n, C * ic_bn + c, d, h, w], name="data_vec"
    )
    ...
    return data_vec, kernel_vec

def conv3d_ncdhw(data, kernel, strides, padding, dilation, out_dtype):
    layout = "NCDHW"
    packed_out = conv3d_NCDHWc(data, kernel, strides, padding, dilation, layout, out_dtype)
    return unpack_NCDHWc_to_ncdhw(packed_out, out_dtype)
def _conv3d_NCDHWc(data, kernel, strides, padding, dilation, layout, out_dtype):
    out_dtype = data.dtype if out_dtype is None else out_dtype
    ...
    if len(data.shape)==5:
        """5d tensor:[batch_size, in_channel, in_depth, in_height, in_width]"""
        ...
        batch_size, ic_chunk, in_depth, in_height, in_width, ic_bn = get_const_tuple(data.shape)
    ...
    if DOPAD:
        data_pad = pad(
            data,
            (0, 0, pad_front, pad_top, pad_left, 0),
            (0, 0, pad_back, pad_down, pad_right, 0),
            name="data_pad",
        )
    else:
        data_pad = data
    ...
    return conv
@autotvm.register_topi_compute("conv3d_NCDHWc.x86")
def conv3d_NCDHWc(cfg, data, kernel, strides, padding, dilation, layout, out_dtype):
    """3D convolution forward operator.

    Parameters
    ----------
    input : tvm.te.Tensor
        6-D input data with shapes:
        [batch, in_channel, in_depth, in_height, in_width] for NCDHWc layout

    filter : tvm.te.Tensor
        6-D filter with shape [out_channels, in_channels, kernel_depth, kernel_height, kernel_width]
    ...
    -------
    output : tvm.te.Tensor
        6-D with shape [batch, out_channel//oc_bn, out_depth, out_height, out_width, oc_bn] for NCDHWc layout
    """
    # layout = "NCDHWc"
    out_dtype = data.dtype if out_dtype is None else out_dtype
    strides = strides if isinstance(strides, (tuple, list)) else (strides, strides, strides)
    ...
    if len(dshape) == 5:
        ...
        n, ic_chunk, d, h, w, ic_bn = dshape
        oc_chunk, ic_chunk, kd, kh, kw, ic_bn, oc_bn = kshape
        ic = ic_chunk * ic_bn
        oc = oc_chunk * oc_bn
    # end
    # else:
    ...
    if cfg.is_fallback:
        # _get_default_config(cfg, data, kernel, strides, padding, out_dtype, layout)
        _get_default_config(cfg,
                            te.placeholder((n, ic, d, h, w), dtype=data.dtype),
                            te.placeholder((oc, ic, kd, kh, kw), dtype=kernel.dtype),
                            strides, padding, out_dtype, layout)
    if len(data.shape) == 5:
        dshape = (n, ic // cfg["tile_ic"].size[-1], d, h, cfg["tile_ic"].size[-1], w)
        kshape = (oc // cfg["tile_oc"].size[-1], ic // cfg["tile_ic"].size[-1], kd, kh, kw, cfg["tile_ic"].size[-1], cfg["tile_oc"].size[-1],)
    ...
    return nn.conv3d_NCDHWc(data, kernel, strides, padding, dilation, layout, out_dtype)
@autotvm.register_topi_schedule("conv3d_NCDHWc.x86")
def schedule_conv3d_NCDHWc(cfg, outs):
    """TOPI schedule callback for conv3d
    ...
    def _traverse(op):
        if "conv3d_NCDHWc" in op.tag:
            output = op.output(0)
            conv_out = op.output(0)
            kernel_vec = output.op.input_tensors[1]
            data_vec = output.op.input_tensors[0]
            args = [s, cfg, data_vec, kernel_vec, conv_out, output, outs[0]]
            _schedule_conv3d_NCDHWc(*args)

    traverse_inline(s, outs[0].op, _traverse)
    return s

def _schedule_conv3d_NCDHWc(s, cfg, data_vec, kernel_vec, conv_out, output, last):
    # fetch schedule
    ...
    if isinstance(s[data_vec].op, tvm.te.ComputeOp) and "pad" in data_vec.op.tag:
        batch, ic_chunk, id, ih, ic_block, iw = s[data_vec].op.axis
        # s[data_vec].vectorize(ic_block)
        parallel_axis = s[data_vec].fuse(batch, ic_chunk, id, ih)
    ...
    if isinstance(kernel_vec.op, tvm.te.ComputeOp) and kernel_vec.name == "kernel_vec":
        ...
        oc_chunk, ic_chunk, od, oh, ow, ic_block, oc_block = s[kernel_vec].op.axis
        s[kernel_vec].reorder(oc_chunk, od, oh, ic_chunk, ow, ic_block, oc_block)
        if oc_bn > 1:
            s[kernel_vec].vectorize(oc_block)
        parallel_axis = s[kernel_vec].fuse(oc_chunk, od, oh)
    ...
    if C != O:
        ...
        if out_ndim == 5:
            ...
            batch, oc_chunk, od, oh, ow, oc_block = s[O].op.axis
            ow_chunk, ow_block = s[O].split(ow, factor=reg_n)
            s[O].reorder(oc_chunk, od, oh, ow_chunk, ow_block, oc_block)
            parallel_axis = s[O].fuse(batch, oc_chunk, od, oh)
            s[C].compute_at(s[O], parallel_axis)
...
def unpack_NCDHWc_to_ncdhw(packed_out, out_dtype):
    n, oc_chunk, od, oh, ow, oc_bn = get_const_tuple(packed_out.shape)
    unpack_shape = (n, oc_chunk*oc_bn, od, oh, ow)
    idxmod = tvm.tir.indexmod
    idxdiv = tvm.tir.indexdiv
    unpacked_out = te.compute(
        unpack_shape,
        lambda n, d, h, w, c: packed_out[n, idxdiv(c, oc_bn), d, h, w, idxmod(c, oc_bn)].astype(
            out_dtype
        ),
        name="output_unpack",
...
@conv3d_infer_layout.register("cpu")
def _conv3d_infer_layout(workload, cfg):
    _, data, kernel, strides, padding, dilation, layout, dtype = workload
    batch_size, in_channel, in_depth, in_height, in_width = data[1]
    out_channel, _, k_depth, k_height, k_width = kernel[1]
    DSTR, HSTR, WSTR = strides
    idxdiv = tvm.tir.indexdiv
    assert isinstance(dilation, int) or len(dilation) == 3
    if isinstance(dilation, int):
        ...
        dilation_d, dilation_h, dilation_w = dilation
    dilated_kernel_d = (k_depth - 1) * dilation_d + 1
    ...
    in_shape = (batch_size, idxdiv(in_channel, tile_ic), in_depth, in_height, in_width, tile_ic)
    in_layout = "NCDHW%dc" % tile_ic
    out_shape = (batch_size, idxdiv(out_channel, tile_oc), out_depth, out_height, out_width, tile_oc)
    out_layout = "NCDHW%dc" % tile_oc
...
def _get_default_config(cfg, data, kernel, strides, padding, out_dtype, layout):
    ...
    """
    # modify
    if layout not in ["NDHWC", "NCDHW", "NCDHWc"]:
...
def _get_conv3d_workload(data, kernel, stride, padding, out_dtype, data_layout="NCHW"):
    ...
    # modify
    elif data_layout == "NCDHWc":
        if len(data.shape)==5:
            _, CI, ID, IH, IW = get_const_tuple(data.shape)
            CO, CIG, KD, KH, KW = get_const_tuple(kernel.shape)
        else:
            _, CI_CHUNK, ID, IH, IW, CI_BN = get_const_tuple(data.shape)
            CO_CHUNK, CI_CHUNK, KD, KH, KW, CI_BN, CO_BN = get_const_tuple(kernel.shape)
            CI = CI_CHUNK * CI_BN
            CO = CO_CHUNK * CO_BN
            CIG = CI_CHUNK * CI_BN
    # end
